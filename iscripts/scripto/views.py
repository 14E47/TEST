...
from .forms import UploadFileForm, ClientForm
    ...
    try:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return render(request, 'scripto/script7.html', args)
            return render(request, 'scripto/fail.html')
        else:
            form = UploadFileForm()
        return render(request, 'scripto/upload/uploadFile.html', {'form': form})
...
def script7Part2(request):
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                print('gammma')
                file_loc = request.POST['file_url']
                input_colname = request.POST['input_column_name']
                output_colname = request.POST['output_column_name']
                commondata = request.POST['common_data']
                split = request.POST['split_checkbox']
                str_start = request.POST['start_str']
                str_end = request.POST['end_str']
                seperator = request.POST['deliminator']
                df = pd.read_excel(file_loc)

                processed = []
                not_processed = []
                for i in df[input_colname]:
                    z = re.search('%s(.*)%s' % (str_start, str_end), i)
                    req_str = z.group(1)
                    print(req_str)
                    if seperator == "01":
                        val = req_str.split(' ')
                        if len(val) == 1:
                            not_processed.append(val[0])
                        else:
                            processed.append(val[0])
                    elif seperator == "02":
                        val = req_str.split('   ')
                        if len(val) == 1:
                            not_processed.append(val[0])
                        ...
                        val = re.findall('[A-Z][^A-Z]*', req_str)
                        if len(val) == 1:
                            not_processed.append(val[0])
                        else:
                            processed.append(val[0])
                ...
                ndf1[output_colname] = processed
                print(ndf1)
                ndf2['not processed'] = not_processed
                print(ndf2)

                ndf1[output_colname].to_excel(os.path.join(os.path.dirname(__file__), "../output/script7/processed/processed.xlsx"), index=False)
                print('ndf1 written')
                ndf2['not processed'].to_excel(os.path.join(os.path.dirname(__file__), "../output/script7/not-processed/not-processed.xlsx"), index=False)
                print('ndf2 written')

                os.remove(file_loc)
                return render(request, 'scripto/script7-2.html')
            return render(request, 'scripto/fail.html')
        else:
            form = ClientForm()
        return render(request, 'scripto/upload/uploadFile.html', {'form': form})
...
def ds71(request):
    dpath = os.path.join(os.path.dirname(__file__), "../output/script7/processed/processed.xlsx")
    down = download(request, dpath)
    return down

def ds72(request):
    dpath = os.path.join(os.path.dirname(__file__), "../output/script7/not-processed/not-processed.xlsx")
    down = download(request, dpath)
    return down
...
#         else:
#             form = SampleForm()
#         return render(request, 'scripto/upload/script7upoadFile.html', {'form': form})
#     except:
#         raise Http404